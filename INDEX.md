...
| `templates/skills/onboarding-guidance/SKILL.md` | The Designer's operating knowledge for guidance work (procedure, judgment, learning loop) | Any guidance/tour/onboarding work — loads with the module |
| `templates/skills/functional-site-audit/` | Reverse-engineer a reference product's flows into a functional analysis + actionable PRD (packaged skill + 5 extraction scripts + long-form playbook in `references/`) | Studying a competitor or reference product |
| `templates/visual-review/README.md` | Visual surfaces for the [HUMAN] gates: interactive plans, diff recaps, repo docs, design directions, live-app flow review — external skills adopted by reference, privacy defaults + demotion cue inside | Any gate whose reviewer is the human owner and whose decision has structure worth visualizing |
| `templates/onboarding/README.md` | Workflow-first user guidance: guidance-moments map, mechanism ladder, UX guardrails, driver.js wrapper rules, mandatory tour events | Product has/nears real users; the launch first-run item; recurring user confusion |
