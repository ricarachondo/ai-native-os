...
## 2026-08 · An inventory of an active system was stale before it was delivered

**What happened**: a forensic inventory was produced to plan a repository
consolidation — asset by asset, every claim verified. It was accurate when
written. Within the same session it was wrong twice: the two repositories
took **13 more commits while the document was being written**, a branch
listed as "in flight" merged, and a worktree named in the plan had been
replaced by a different one. The user caught it, not the process: *"the
repos have been worked on in parallel while we talk."*

**Why it matters more than it sounds**: nothing in the document announced
that it had expired. A stale fact and a fresh one look identical on the
page, and a plan that opens with "verified" invites being trusted at
execution time — which is precisely when the volatile half is oldest.

**Root cause**: the inventory mixed two kinds of fact with the same
authority. **Durable** facts (two systems share one database; the coupling
runs both ways; one tracker serves both; this code is duplicated by hand)
do not change with commits and can be trusted for weeks. **Volatile**
facts (commit counts, branches, worktrees, untracked files, reference
counts) expire in hours on an active codebase. Presenting both as
"verified evidence" made the whole document age at the speed of its
fastest-rotting part.

**What changed**: any plan built on an inventory of a LIVE system now
declares the split, and the volatile half is **not read from the
document** — it is regenerated by a read-only script at the moment of
execution, which prints GO / GO-WITH-CONDITIONS / NO-GO and whose output
is pasted into the tracker as the record. Its first run immediately caught
three more drifts, including a NO-GO that the written plan would have
walked straight past.

**Generalizable**: before writing "verified" next to a fact, ask how long
